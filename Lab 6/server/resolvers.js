...
export const resolvers = {
    Query: {
        ...
        getArtistById: async (_, args) => {
            const artists = await artistCollection();
            const response = await client.exists(args._id);
            if (response == 1){
                const artist = await client.get(args._id)
            ...
            await client.set(args._id, JSON.stringify(artist))
            return artist;
        },
        getAlbumById: async (_, args) => {
            ...
            const response = await client.exists(args._id);
            if (response){
                const album = await client.get(args._id)
            ...
            await client.set(args._id, JSON.stringify(album))
        ...
        getCompanyById: async (_, args) => {
            ...
            const response = await client.exists(args._id);
            if (response){
                const company = await client.get(args._id)
            ...
            await client.set(args._id, JSON.stringify(company))
    ...
    Album:{
        artist: async (parentValue) => {
            const artists = await artistCollection();
            const artistId = new ObjectId(parentValue.artistId)
            let artist = await artists.findOne({_id: artistId });
            return artist
        },
        recordCompany: async (parentvalue) => {
            const recordCompanies = await recordcompaniesCollection();
            const companyId = new ObjectId(parentvalue.recordCompanyId)
            let recordCompany = await recordCompanies.findOne({_id: companyId})
    ...
    Mutation: {
        ...
        editArtist: async(_, args) => {
            ...
            await artists.findOneAndUpdate({_id: searchTerm}, {$set: newArtist})
            if(client.exists("allArtists")){
                client.del("allArtists")
            }
            if(client.exists(args._id)){
        ...
        removeArtist: async (_, args) => {
            ...
            allCompanies.forEach((recordCompany) => {
                ...
                recordCompanies.findOneAndUpdate({_id: companyId}, {$set: {albums: newAlbums}})
            })
        ...
        editCompany: async(_, args) => {
            ...
            await companies.findOneAndUpdate({_id: searchTerm}, {$set: newCompany})
            if(client.exists(args._id)){
                await client.del(args._id)
            }
            if(client.exists("allCompanies")){
            ...
            for(let artist of artistArray){
                ...
                for(let album of artistAlbums){
                    ...
                    if(recordCompany === searchTerm.toString()){
                        await artists.findOneAndUpdate({_id: new ObjectId(artist._id)}, {$pull: {albums: new ObjectId(albumDoc._id)}})
                    }
                }
            }
        ...
        editAlbum: async(_, args) => {
            ...
            if(args.songs){
                for (const song of args.songs){
                    if(!song || typeof song !== "string" || song.trim().length === 0 || !/^[a-zA-Z\s]*$/.test(song)){
                        throw new GraphQLError('Songs must be nonempty strings containing only letters!', {
                            extensions: {code: "BAD_USER_INPUT"},
                        })
                    }
                }
            ...
            await albums.findOneAndUpdate({_id: searchTerm}, {$set: newAlbum})
            
            if(args.artistId){
                await artists.findOneAndUpdate(
                    {_id: new ObjectId(args.artistId)},
                    {$push: {albums: searchTerm}}
                )
            }
                if(args.companyId){
                await companies.findOneAndUpdate(
